 ...
 * in which case Witty will introspect the record to build the
 * corresponding URL.
 *
 ...
 *
 * When taking advantage of model awareness, be sure to configure Witty
 ...
 *
 *     this.datastore(require('witty-mongoose'));
 *
...
function urlFor(req) {
  ...
  return function(options) {
    ...
    options.controller = router.util.controllerize(options.controller) || req._maglev.controller;
    options.action = router.util.functionize(options.action) || req._maglev.action;
