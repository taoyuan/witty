 ...
 * Examples:
 *
 *   app.phase(witty.boot.httpServer(8080));
 *
 *   app.phase(witty.boot.httpServer(8080, '127.0.0.1'));
 *
 *   app.phase(witty.boot.httpServer({ address: '127.0.0.1', port: 8080 }));
 *
 * @param {Number} port
...
module.exports = function(port, address, options) {
  ...
  return function httpServer(done) {
    ...
    address = address || options.address || this.get('host') || this.get('address') || '0.0.0.0';
