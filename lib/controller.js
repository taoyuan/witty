...
Controller.prototype.params = function (name, defaultValue) {
  ...
  if ((name in params) && params.hasOwnProperty(name)) { return params[name]; }
...
Controller.prototype.render = function(template, options, fn) {
  ...
  // delete option keys consumed by Witty
 ...
 *     })
 *
 * By default Witty passes an `Error` with a `.status` of 406 to
...
/**
 * Invoked when controller is initialized by Witty.
 *
 * This function is called immediately after Witty instantiates the
...
Controller.prototype._invoke = function(action) {
  ...
  // Assign Witty properties to request.  These properties are used by
  // helper functions, such as urlFor(), to return results based on the context
  // of the request and response.
  this.__req._maglev = {};
  this.__req._maglev.app = this.__app;
  this.__req._maglev.controller = this.__id;
  this.__req._maglev.action = this.__action;
  ...
  for (key in dynamicHelpers) {
    if (dynamicHelpers.hasOwnProperty(key)) {
