...
GeneratorUtilities.prototype.createDirectory = function (dir) {
    ...
    if (fs.existsSync(path.join(this.baseDir, dir))) {
        this.log($.bold.bold('exists') + '  ' + displayDir);
    } else {
        fs.mkdirSync(path.join(this.baseDir, dir), 0755);
        this.log($.bold.green('create') + '  ' + displayDir);
    }
};
...
GeneratorUtilities.prototype.copyTemplate = function(fileName, destFileName, variables, templateData) {
  var pkg = this.options.tpl + '-ext';
  var tpl;
  try {
      tpl = require(pkg);
  } catch(e) {
      return console.log('Package ' + pkg + ' required to generate views');
  }
  ...
  if(fs.existsSync(fileDestPath)) {
    this.log($.bold.grey('exists') + '  ' + displayFileName);
  } else {
    ...
    this.log($.bold.green('create') + '  ' + displayFileName);
  }
};
...
GeneratorUtilities.prototype.copyFile = function (fileName, destFileName, variables) {
  ...
  try {
    ...
    this.log($.bold.red('error') + '   ' + displayFileName);
  }
};
...
GeneratorUtilities.prototype.appendFileToFile = function(fileName, destFileName, variables) {
  ...
  var content;
  try {
    content = fs.readFileSync(destFilePath).toString();
  } catch(e) {
      return;
  }
  ...
  this.log($.bold.blue('patch') + '   ' + displayFileName);
};
