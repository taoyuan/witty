...
exports = module.exports = function server(dir, address, port, env, options) {
  ...
  // If debug mode is enabled, Witty will respawn using a node process with
  ...
  console.log('Witty %s application starting in %s on http://%s:%d', witty.version
                                                                        , env, address, port);

  debug('booting app at %s in %s environment', dir, env);

  var app = witty;
  app.phase(witty.boot.controllers(path.resolve(dir, './app/controllers')));
  ...
  app.phase(witty.boot.routes(path.resolve(dir, './config/routes')));
  app.phase(witty.boot.httpServer(port, address));
